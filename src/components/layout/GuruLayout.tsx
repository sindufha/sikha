...
import {
  LayoutDashboard, ClipboardCheck, FileText, LogOut, Menu, School, ChevronDown, Camera, UserCircle,
} from 'lucide-react'
...
const navItems = [
  ...
  { to: '/guru/presensi-manual', icon: ClipboardCheck, label: 'Presensi Manual' },
  { to: '/guru/presensi-qr', icon: Camera, label: 'Scan QR' },
  { to: '/guru/laporan', icon: FileText, label: 'Laporan Presensi' },
  { to: '/guru/profil', icon: UserCircle, label: 'Profil' },
]
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={cn(
        ...
        <div className="h-14 flex items-center gap-2.5 px-4 border-b border-white/10 shrink-0">
          <div className="w-8 h-8 rounded-lg bg-secondary flex items-center justify-center shadow-sm shadow-secondary/30">
        ...
        <div className="p-2.5 border-t border-white/10">
          <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-lg">
            <div className="w-7 h-7 rounded-full bg-secondary/20 flex items-center justify-center shrink-0">
              <span className="text-xs font-bold text-secondary-light">{user?.nama?.charAt(0) || 'G'}</span>
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold text-slate-200 truncate">{user?.nama || 'Guru'}</p>
          ...
          <div ref={userMenuRef} className="relative shrink-0">
            ...
            >
              <div className="w-6 h-6 rounded-md bg-secondary flex items-center justify-center">
                <span className="text-[10px] font-bold text-white">{user?.nama?.charAt(0) || 'G'}</span>
              </div>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-52 rounded-xl border border-border bg-white py-1.5 shadow-dropdown animate-in">
                <div className="px-3.5 py-2 border-b border-border">
                  ...
                  <p className="text-xs text-text-muted">Guru</p>
                </div>
